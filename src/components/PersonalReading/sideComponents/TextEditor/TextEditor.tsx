...
export class WrapperTextEditor extends React.Component<
  {
    readOnly: number;
    markdownValue: JSONRecord;
    commitIdx: number;
    userIdx: string;
    pdfIdx: string;
  },
...
function updateEditor(userID: string, pdfID: string, value: any) {
  timerId = setTimeout(() => {
    socket.emit(
      'updateEditor',
      { id: userID, pdfId: pdfID, text: value },
      clearTimeout(timerId)
    );
  }, 700);
}

async function updateOrClearEditor(
  userID: string,
  pdfID: string,
  jsonOutput: any
) {
...
async function getPresignedUrl(userID: string) {
  try {
    ...
    const response = await axios.post(
      'https://junglenewcumbersocket.shop:443/users/sign',
      {
        fileName,
      }
    );
...
export const TextEditor: FC<{
  ...
  userIdx: string;
  pdfIdx: string;
}> = ({ readOnly, value, markdownValue, commitIdx, userIdx, pdfIdx }) => {
  // let readonly = false;
            ...
            for (let i = 0; i < arrOutput.length; i++) {
              ...
              if (arrOutput[i].content) {
                for (let j = 0; j < arrOutput[i].content.length; j++) {
                  ...
                  if (
                    contentAttr !== undefined &&
                    !contentAttr['alt'] &&
                    contentInDocType === 'image'
                  ) {
  ...
  useEffect(() => {
    document
      ...
      ?.setAttribute('ondrop', 'drop_handler(event)');
    document
      .querySelector('.ProseMirror.editor')
      ?.setAttribute('ondragover', 'dragover_handler(event)');

    let height = document.querySelector(
      '.PersonalReading__mainPage'
    )?.clientHeight;
    if (height != null) {
      document
        .querySelector('.milkdown')
        ?.setAttribute('style', `height: ${height - 240}px`);
    }
